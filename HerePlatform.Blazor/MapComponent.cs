...
public class MapComponent : ComponentBase, IAsyncDisposable
{
    ...
    private IHerePlatformKeyService? _keyService;
    internal event EventHandler? MapInitialized;

    protected override void OnInitialized()
    {
        _keyService = ServiceProvider.GetService<IHerePlatformKeyService>();
        base.OnInitialized();
    }
    ...
    public async Task InitAsync(ElementReference element, MapOptions? options = null)
    {
        if (options?.ApiLoadOptions == null && _keyService != null)
        {
            ...
            try
            {
                isHereReady = await JsRuntime.InvokeAsync<bool>(JsInteropIdentifiers.CanRenderMap);
            }
            catch (Exception ex)
            {
