window.herePlatform = window.herePlatform || {};

window.herePlatform.objectManager = function () {
    ...
    // e.g. "_content/HerePlatform.NET.Blazor/" from "…/js/objectManager.js"
    ...
    function removeObjectFromMap(id) {
        ...
        if (mapId) {
            ...
            if (map instanceof H.Map) {
                try { map.removeObject(obj); } catch (e) { console.debug('[HerePlatform]', e.message || e); }
                return;
            }
        }
        ...
        } catch (e) { console.debug('[HerePlatform]', e.message || e); }
    }
    ...
    function removeLayerFromMap(id) {
        ...
        if (mapId) {
            ...
            if (map && typeof map.removeLayer === 'function') {
                try { map.removeLayer(wrapper.layer); } catch (e) { console.debug('[HerePlatform]', e.message || e); }
            }
        }
    }
        ...
        if (mapId) {
            ...
            if (map) {
                ...
                if (uiGuid && mapObjects[uiGuid]) {
                    try { mapObjects[uiGuid].removeBubble(bubble); } catch (e) { console.debug('[HerePlatform]', e.message || e); }
                }
            }
        }
    }
    ...
    function extractShapeGeometry(obj, objectType) {
        try {
            ...
            console.warn('[HerePlatform] Failed to extract shape geometry:', e);
        }
        return null;
    }
    ...
    return {
        resetPlatform: function () {
            ...
            for (const guid of Object.keys(mapObjects)) {
                ...
                if (obj && typeof obj.dispose === 'function') {
                    try { obj.dispose(); } catch (e) { console.debug('[HerePlatform]', e.message || e); }
                }
            }
        ...
        initMap: async function (apiOptions) {
            ...
            if (!key || key === 'YOUR_API_KEY' || key.trim() === '') {
                console.warn('[HerePlatform] No valid HERE API key configured. Map will not be initialized. ' +
                    'Set a valid API key via services.AddHerePlatformBlazor("your-key") or in appsettings.json.');
                apiKeyValid = false;
                return;
            }
            ...
            try {
                ...
                if (defaultLayerOpts.engineType) {
                    console.warn('[HerePlatform] HARP engine failed, falling back to default engine.');
        ...
        createHereMap: function (container, options) {
            ...
            if (!apiKeyValid || !herePlatform) {
                ...
                container.innerHTML = '<div><strong>HERE Map</strong><br/>No valid API key configured.<br/>' +
                    '<small>Set your key via <code>services.AddHerePlatformBlazor("your-key")</code><br/>' +
        ...
        createObject: function (args) {
            if (!isHereApiLoaded()) {
                console.warn('[HerePlatform] HERE API not loaded. Cannot create object.');
                return null;
            }
            ...
            if (!allowedPrefixes.some(p => constructorName.startsWith(p))) {
                console.error(`[HerePlatform] Constructor '${constructorName}' not allowed.`);
                return null;
            }
            ...
            try {
                ...
                if (prevOverlays && prevOverlays.length > 0) {
                    for (const ol of prevOverlays) {
                        try { map.removeLayer(ol); } catch (e) { console.debug('[HerePlatform]', e.message || e); }
                    }
                }
                map['_blzOverlays'] = [];
                ...
                if (parts[0] === 'vector' && parts[1] === 'traffic') {
                    ...
                    for (const part of parts) {
                        ...
                        layer = layer[part];
                        if (layer === undefined) {
                            console.warn('[HerePlatform] Layer path "' + layerPath + '" not found. Available keys:', Object.keys(prevLayer));
                            return;
                        }
                    }
                    if (layer) {
                ...
                if (parts[0] === 'hybrid') {
                    ...
                    if (!hybridGroup) {
                        console.warn('[HerePlatform] Unknown hybrid group:', group);
                        return;
                    }
                    ...
                    for (const part of parts) {
                        ...
                        if (layer === undefined) {
                            console.warn('[HerePlatform] Layer path "' + layerPath + '" failed at "' + resolvedPath + '". Available keys:', Object.keys(prevLayer));
                            return;
                        }
                    }
                    if (layer) {
                ...
                console.warn('[HerePlatform] Could not set base layer:', layerPath, e);
            }
        },
        ...
        addOverlayLayer: function (mapGuid, layerPath) {
            ...
            try {
                ...
                console.warn('[HerePlatform] Could not add overlay layer:', layerPath, e);
            }
        },
        ...
        removeOverlayLayer: function (mapGuid, layerPath) {
            ...
            try {
                ...
                console.warn('[HerePlatform] Could not remove overlay layer:', layerPath, e);
            }
        },
        ...
        getViewBounds: function (mapGuid) {
            ...
            } catch (e) { console.warn('[HerePlatform] getViewBounds failed:', e); }
            return null;
        },
        ...
        setViewBounds: function (mapGuid, bounds) {
            ...
            } catch (e) { console.warn('[HerePlatform] setViewBounds failed:', e); }
        },

        resizeMap: function (mapGuid) {
            const map = mapObjects[mapGuid];
            if (!map || map._blzPlaceholder) return;
            try { map.getViewPort().resize(); } catch (e) { console.debug('[HerePlatform]', e.message || e); }
        },

        invokeWithReturnedObjectRef: async function (args) {
            const result = await herePlatform.objectManager.invoke(args);
            const uuid = uuidv4();
        ...
        disposeMultipleObjects: function (guids) {
            ...
            for (const guid of guids) {
                herePlatform.objectManager.disposeObject(guid);
            }
        },
        ...
        disposeMap: function (mapGuid) {
            ...
            if (map) {
                ...
                if (behGuid && mapObjects[behGuid]) {
                    try { mapObjects[behGuid].dispose(); } catch (e) { console.debug('[HerePlatform]', e.message || e); }
                ...
                if (autoBubble) {
                    ...
                    if (uiForBubble) {
                        try { uiForBubble.removeBubble(autoBubble); } catch (e) { console.debug('[HerePlatform]', e.message || e); }
                    }
        ...
        setViewportPadding: function (mapGuid, top, right, bottom, left) {
            ...
            try {
                ...
                console.warn('[HerePlatform] Failed to set viewport padding:', e);
            }
        },
        ...
        updateMarkerComponent: function (id, options, callbackRef) {
            ...
            if (iconUrl) {
                try {
                    ...
                    console.warn('[HerePlatform] Failed to create icon from URL:', e);
                }
            } else if (draggable) {
                try {
                    ...
                    console.warn('[HerePlatform] Failed to create draggable icon:', e);
                }
            }
            ...
            if ((minZoom != null || maxZoom != null) && map) {
                var checkZoomVisibility = function () {
                    ...
                    try { marker.setVisibility(show); } catch (e) { console.debug('[HerePlatform]', e.message || e); }
                };
            ...
            if (infoBubbleTemplateId && map) {
                marker.addEventListener('tap', function () {
                    ...
                    if (prev) {
                        try { ui.removeBubble(prev); } catch (e) { console.debug('[HerePlatform]', e.message || e); }
                    }
        ...
        disposeMarkerComponent: function (id) {
            ...
            herePlatform.objectManager.disposeObject(id);
        },
        ...
        groupRemoveObjects: function (groupId, objectIds) {
            ...
            for (const objId of objectIds) {
                const obj = mapObjects[objId];
                if (obj) {
                    try { group.removeObject(obj); } catch (e) { console.debug('[HerePlatform]', e.message || e); }
                }
            }
        },
                ...
                fetch('https://geocode.search.hereapi.com/v1/geocode?q=test&limit=1&apiKey='
                    ...
                    .then(function (resp) {
                        if (resp.status === 401 || resp.status === 403) {
                            console.error('[HerePlatform] HERE API auth failed. Tiles will not load. HTTP ' + resp.status);
        ...
        addMultipleListeners: async function (args) {
            ...
            await Promise.all(guids.map((guid, index) => {
                ...
                return herePlatform.objectManager.invoke(args3);
            }));
        ...
        createMultipleObject: function (args) {
            ...
            for (let i = 0; i < guids.length; i++) {
                ...
                herePlatform.objectManager.createObject(objArgs);
            }
        },
        ...
        invokeMultiple: async function (args) {
            ...
            const promises = guids.map((guid, index) => {
                ...
                const result = herePlatform.objectManager.invoke(args3);
            ...
            return new Promise(function (resolve, reject) {
                ...
                router.calculateRoute(params, function (result) {
                    ...
                    if (isAuthError(error)) {
                        console.error('[HerePlatform] HERE API auth failed (401/403). Check API key. Service: routing');
                        reject(new Error('HERE_AUTH_ERROR:routing:' + (error.message || error)));
                    } else {
                        console.warn('[HerePlatform] Routing error:', error);
                        resolve({ routes: [] });
                    }
                });
            });
        ...
        geocode: function (query, options) {
            return new Promise(function (resolve, reject) {
                ...
                service.geocode(params, function (result) {
                    ...
                    if (isAuthError(error)) {
                        console.error('[HerePlatform] HERE API auth failed (401/403). Check API key. Service: geocoding');
                        reject(new Error('HERE_AUTH_ERROR:geocoding:' + (error.message || error)));
                    } else {
                        console.warn('[HerePlatform] Geocode error:', error);
                        resolve({ items: [] });
                    }
                });
            });
        ...
        reverseGeocode: function (position, options) {
            return new Promise(function (resolve, reject) {
                ...
                service.reverseGeocode(params, function (result) {
                    ...
                    if (isAuthError(error)) {
                        console.error('[HerePlatform] HERE API auth failed (401/403). Check API key. Service: reverse-geocoding');
                        reject(new Error('HERE_AUTH_ERROR:reverse-geocoding:' + (error.message || error)));
                    } else {
                        console.warn('[HerePlatform] Reverse geocode error:', error);
                        resolve({ items: [] });
                    }
                });
            });
        ...
        updateMarkerClusterComponent: async function (id, options, callbackRef) {
            ...
            if (!H.clustering) {
                ...
                if (!H.clustering) {
                    console.warn('[HerePlatform] HERE clustering module not available after loading mapsjs-clustering.js.');
                    return;
                }
            }
        ...
        updateGeoJsonReaderComponent: async function (id, options, callbackRef) {
            ...
            if (!H.data || !H.data.geojson) {
                ...
                if (!H.data || !H.data.geojson) {
                    console.warn('[HerePlatform] HERE data module not available after loading mapsjs-data.js.');
                    return;
                }
            }
        ...
        autosuggest: function (guid, query, options, callbackRef) {
            ...
            service.autosuggest(params, function (result) {
                ...
                if (mapObjects[guid] !== state || state.gen !== gen) return;
                if (isAuthError(error)) {
                    console.error('[HerePlatform] HERE API auth failed (401/403). Check API key. Service: autosuggest');
        ...
        updateHeatmapComponent: async function (id, options, callbackRef) {
            ...
            if (typeof H === 'undefined' || !H.data || !H.data.heatmap) {
                ...
                if (!H.data || !H.data.heatmap) {
                    console.warn('[HerePlatform] HERE data/heatmap module not available after loading mapsjs-data.js.');
                    return;
                }
            }
        ...
        getTrafficIncidents: async function (north, south, east, west) {
            ...
            try {
                ...
                if (!resp.ok) {
                    if (resp.status === 401 || resp.status === 403) {
                        console.error('[HerePlatform] HERE API auth failed. Service: traffic-incidents. HTTP ' + resp.status);
                        throw new Error('HERE_AUTH_ERROR:traffic-incidents:HTTP ' + resp.status);
                    }
                    console.warn('[HerePlatform] Traffic API v7 incidents error:', resp.status);
                    return { incidents: [] };
                }
                ...
                console.warn('[HerePlatform] Traffic incidents error:', error);
                return { incidents: [] };
            }
        },
        ...
        getTrafficFlow: async function (north, south, east, west) {
            ...
            try {
                ...
                if (!resp.ok) {
                    if (resp.status === 401 || resp.status === 403) {
                        console.error('[HerePlatform] HERE API auth failed. Service: traffic-flow. HTTP ' + resp.status);
                ...
                if (error && error.message && error.message.indexOf('HERE_AUTH_ERROR') !== -1) throw error;
                console.warn('[HerePlatform] Traffic flow error:', error);
                return { items: [] };
            }
        },
        ...
        getTransitDepartures: function (position) {
            return new Promise(function (resolve, reject) {
                ...
                service.getDepartures(params, function (result) {
                    ...
                    if (isAuthError(error)) {
                        console.error('[HerePlatform] HERE API auth failed (401/403). Check API key. Service: transit-departures');
                        reject(new Error('HERE_AUTH_ERROR:transit-departures:' + (error.message || error)));
                    } else {
                        console.warn('[HerePlatform] Transit departures error:', error);
                        resolve({ departures: [] });
                    }
                });
            });
        ...
        searchTransitStations: function (position, radiusMeters) {
            return new Promise(function (resolve, reject) {
                ...
                service.getStations(params, function (result) {
                    ...
                    if (isAuthError(error)) {
                        console.error('[HerePlatform] HERE API auth failed (401/403). Check API key. Service: transit-stations');
                        reject(new Error('HERE_AUTH_ERROR:transit-stations:' + (error.message || error)));
                    } else {
                        console.warn('[HerePlatform] Transit stations error:', error);
                        resolve({ stations: [] });
                    }
                });
            });
        ...
        updateOverviewMap: function (id, options, callbackRef) {
            ...
            try {
                ...
                if (!baseLayer) {
                    console.warn('[HerePlatform] Overview: no base layer available');
                    return;
                }
        ...
        optimizeWaypointSequence: function (request) {
            return new Promise(function (resolve, reject) {
                ...
                router.calculateRoute(params, function (result) {
                    ...
                    if (isAuthError(error)) {
                        console.error('[HerePlatform] HERE API auth failed (401/403). Check API key. Service: waypoint-sequence');
                        reject(new Error('HERE_AUTH_ERROR:waypoint-sequence:' + (error.message || error)));
                    } else {
                        console.warn('[HerePlatform] Waypoint sequence error:', error);
                        resolve({
                            optimizedIndices: [],
                            optimizedWaypoints: [],
                            totalDistance: 0,
        ...
        discoverPlaces: function (request) {
            return new Promise(function (resolve, reject) {
                ...
                service.discover(params, function (result) {
                    resolve(herePlatform.objectManager._mapPlacesResult(result));
                }, function (error) {
                    if (isAuthError(error)) {
                        console.error('[HerePlatform] HERE API auth failed (401/403). Check API key. Service: discover-places');
                        reject(new Error('HERE_AUTH_ERROR:discover-places:' + (error.message || error)));
                    } else {
                        console.warn('[HerePlatform] Discover places error:', error);
                        resolve({ items: [] });
                    }
                });
            });
        ...
        browsePlaces: function (request) {
            return new Promise(function (resolve, reject) {
                ...
                service.browse(params, function (result) {
                    ...
                    if (isAuthError(error)) {
                        console.error('[HerePlatform] HERE API auth failed (401/403). Check API key. Service: browse-places');
                        reject(new Error('HERE_AUTH_ERROR:browse-places:' + (error.message || error)));
                    } else {
                        console.warn('[HerePlatform] Browse places error:', error);
                        resolve({ items: [] });
                    }
                });
            });
        ...
        lookupPlace: function (request) {
            return new Promise(function (resolve, reject) {
                ...
                service.lookup(params, function (result) {
                    ...
                    var item = herePlatform.objectManager._mapSinglePlaceItem(result);
                    resolve({ items: item ? [item] : [] });
                }, function (error) {
                    if (isAuthError(error)) {
                        console.error('[HerePlatform] HERE API auth failed (401/403). Check API key. Service: lookup-place');
                        reject(new Error('HERE_AUTH_ERROR:lookup-place:' + (error.message || error)));
                    } else {
                        console.warn('[HerePlatform] Lookup place error:', error);
                        resolve({ items: [] });
                    }
                });
            });
        ...
        _mapPlacesResult: function (result) {
            var items = (result.items || []).map(function (item) {
                return herePlatform.objectManager._mapSinglePlaceItem(item);
        ...
        calculateIsoline: function (request) {
            return new Promise(function (resolve, reject) {
                ...
                router.calculateIsoline(params, function (result) {
                                ...
                                isoline.polygons.forEach(function (polygon) {
                                    ...
                                    if (polygon.outer) {
                                        ...
                                        try {
                                            ...
                                            console.warn('[HerePlatform] Failed to decode isoline polyline:', e);
                                        }
                                    }
                                    isolines.push({
                    ...
                    if (isAuthError(error)) {
                        console.error('[HerePlatform] HERE API auth failed (401/403). Check API key. Service: isoline');
                        reject(new Error('HERE_AUTH_ERROR:isoline:' + (error.message || error)));
                    } else {
                        console.warn('[HerePlatform] Isoline error:', error);
                        resolve({ isolines: [] });
                    }
                });
            });
        ...
        calculateMatrix: async function (request) {
            if (!hereApiKey) {
                console.warn('[HerePlatform] No API key for matrix routing.');
            ...
            try {
                ...
                if (resp.status === 401 || resp.status === 403) {
                    console.error('[HerePlatform] HERE API auth failed. Service: matrix-routing. HTTP ' + resp.status);
                ...
                console.warn('[HerePlatform] Matrix routing error:', e);
                return { numOrigins: 0, numDestinations: 0, matrix: [] };
            }
        },
