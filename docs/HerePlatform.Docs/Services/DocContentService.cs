...
public partial class DocContentService : IDocContentService
{
    ...
    private void BuildNavSections()
    {
        ...
        var categoryOrder = new[] { "Getting Started", "Components", "Services", "Utilities", "Guides" };
    ...
    private void BuildSearchEntries()
    {
        ...
        // REST API service pages
        foreach (var def in ServiceDefinitions.All)
        {
            _searchEntries.Add(new SearchEntry
            {
                Slug = def.Href,
                Title = $"{def.Title} (REST API)",
                Headings = ["REST API Explorer"],
                PlainText = $"{def.Title} {def.Description} REST API {def.InterfaceName}"
            });
        }
    }
